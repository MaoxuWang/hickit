...
static void hk_parse_seg(struct hk_seg *s, struct hk_sdict *d, int32_t frag_id, char *str, char *frag_name) // added *frag_name by maoxu
{
	char *p, *q;
	int i, has_digit;

	s->frag_id = frag_id;
	strcpy(s->frag_name, strtok(frag_name, ":")); // added by maoxu
	s->chr = s->st = s->en = -1;
...
parse_seg:		
		for (k = 1; k < n_fields; ++k) {
			...
			hk_parse_seg(&m->segs[m->n_segs++], m->d, m->n_frags, fields[k], fields[0]); // added fields[0] by maoxu, read cell barcode
			++n_segs;
		}
...
void hk_print_pair(FILE *fp, int flag, const struct hk_sdict *d, int32_t n_pairs, const struct hk_pair *pairs)
{
	...
	for (i = 0; i < n_pairs; ++i) {
		...
		fprintf(fp, "%s\t%s\t%d\t%s\t%d\t%c\t%c", p->frag_name, d->name[p->chr>>32], (int32_t)(p->pos>>32), // added p->frag_name by maoxu
